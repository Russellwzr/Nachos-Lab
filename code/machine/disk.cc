...
Disk::Disk(char* name, VoidFunctionPtr callWhenDone, _int callArg)
{
    ...
    handler = callWhenDone;     // 中断处理函数
    handlerArg = callArg;       // 函数参数
    lastSector = 0;             // 最后操作打扇区号
    bufferInit = 0;             // 硬盘最后一次读/写操作发生的时间
    
    fileno = OpenForReadWrite(name, FALSE); //文件模拟硬盘
